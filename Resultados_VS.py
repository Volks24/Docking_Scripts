...
def Histograma(input_file):
    ...
    torsion = []
    ### Histogramas ###
    enegy_df = pd.DataFrame(columns=['rank', 'lowest energy', 'run', 'mean energy', 'cluster' , 'torsion'])
    for j in range(0, len(input_file)):
        if 'Run =' in input_file[j]:
            corrida.append(input_file[j].split('=')[1].strip())
        if 'Torsional Free Energy' in input_file[j]:
            torsion.append(input_file[j].split('=')[1].strip())
        if 'CLUSTERING HISTOGRAM' in input_file[j]:
            torsion = [texto.replace(' kcal/mol', '') for texto in torsion]
            flag = True
            pos = j + 10
            while flag:
                if input_file[pos][0:5] != '_____':
                    data = input_file[pos].split('|')
                    indice = corrida.index((data[2].strip()))
                    valores = [int(data[0]), float(data[1]), int(data[2]), float(data[3]), int(data[4]),torsion[indice]]
...
if __name__ == '__main__':
    ...
    stats_final = pd.DataFrame(columns=['Lig', 'Rank', 'Energy', 'Cluster','Torsion', 'Case'])
    failed_cases = []  # Lista para los casos que fallaron
    stats_corrida = pd.DataFrame(columns=['Lig', 'Normal', 'Bias'])
    for Prot in Lista_Proteinas:
        ...
        for lig in Ligandos:
            ...
            try:
                ...
                shutil.copy(file_name, f'{path_complete}/Resultados/DLG/{file_name}')
                corrida.append('Yes')
            ...
            try:
                ...
                DF_Energia = Histograma(input_file)
                for k in range(0, DF_Energia.shape[0]):
                    stats_final.loc[len(stats_final.index)] = [lig, DF_Energia.iloc[k, 0], DF_Energia.iloc[k, 3], DF_Energia.iloc[k, 4],DF_Energia.iloc[k, 5], 'Bias']
                Grafico_Clusters_Pob(DF_Energia, caso)
                shutil.copy('rank1.pdb', f'{path_complete}/Resultados/{lig}_Bias_rank1.pdb')
                # Mover el archivo DLG_BIAS a la carpeta DLG_BIAS
                shutil.copy(file_name_bias, f'{path_complete}/Resultados/DLG_BIAS/{file_name_bias}')
                corrida.append('Yes')
                os.chdir(path_complete)
            except Exception as e:
        ...
        stats_corrida = pd.concat([stats_corrida, pd.DataFrame([corrida], columns=stats_corrida.columns)], ignore_index=True)
    stats_final.to_csv(f'{path_complete}/Resultados/Energy_cluster_ranks.csv')
    stats_corrida.to_csv(f'{path_complete}/Resultados/Corridas.csv')
